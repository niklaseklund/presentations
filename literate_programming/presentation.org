...
An idea from the presentation to be able to create new code blocks with the same
session and previous block.
#+BEGIN_SRC elisp
(defun my/org-babel-new-last-block ()
  (interactive)
  (save-excursion
    (let ((session-name nil)
          (language nil))
      (while (and (re-search-backward org-babel-src-block-regexp nil t) (not session-name))
        (goto-char (match-beginning 0))
        (let* ((this-info (org-babel-get-src-block-info))
               (this-lang (nth 0 this-info))
               (this-params (nth 2 this-info))
               (this-session (cdr (assoc :session this-params))))
          (when (not (string= "none" this-session))
            (setq session-name this-session)
            (setq language this-lang))))
      (format "%s :session %s" language session-name))))
#+END_SRC

I am thinking that it would work to have a function which can grab the
information and combine it with a ~yasnippet~.

**** Create a custom snippet

First I created a snippets folder as instructed (I got an error not doing that).
Inside ~.config/doom/snippets/org-mode~. I then called ~M-x yas-new-snippet~ to
create a new snippet and manually set the filename to something that could
reside in the newly created folder. I should check if there are any naming
conventions that I should adhere to. I found some interesting [[https://joaotavora.github.io/yasnippet/snippet-development.html][snippet examples]].
