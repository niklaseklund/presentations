...
Devops with Emacs]], in which he talked about literate programming in Emacs using
Org-mode.

He has another blog post, [[http://www.howardism.org/Technical/Emacs/literate-programming-tutorial.html][introduction to Literate Programming]], which inspired
when making this presentation.
...
*** Babel
To make Org-mode sing with literate programming we use an extension called
babel, [[https://orgmode.org/worg/org-contrib/babel/][Babel, active code in Org-mode]]. Babel makes it possible to create code
blocks that can be mixed with regular text.
...
includes ~C++~, ~Python~, ~Shell~ and ~Elisp~. I often find myself in situations
where I want to experiment and document my progress it's great that they all can
be used within the same tool. Another great benefit is that org documents are
plain text documents they are easy to version control and are safe proof for the
future.
...
Oh no ... There is an issue. The application is not compatible with Python3, let's
...
We need to install this function into our Emacs. That is done by adding it to
our ~.emacs.d~ folder. This function makes it possible to provide a name of an
image and get back the current nick-name for that running image.
#+BEGIN_SRC elisp
...
:PROPERTIES:
#+EXPORT_FILE_NAME: original_presentation.pdf
:END:
...
#+NAME: est-truth-data
#+BEGIN_SRC emacs-lisp :results value table :post add-header(*this*) :exports both
  (mapcar (lambda (i)
            (list i (+ (random 4) (- i 2))))
...
It's good to see the data in the table but I often find that some kind of
...
It would be nice to get a KPI like [[https://en.wikipedia.org/wiki/Root-mean-square_deviation][rmse]] to have one number for
...
into in order to be able to calculate the
...
**** Optional
To export with syntax highlight for the code blocks we need to add
#+BEGIN_EXAMPLE
#+LaTeX_HEADER: \usepackage{minted}
#+LaTeX_HEADER: \usemintedstyle{paraiso-light}
#+END_EXAMPLE
To the top of the ~Performance analysis~
...
The conclusion is that the Emacs calc is indeed very powerful and the reputation
of being the [[https://florian.adamsky.it/2016/03/31/emacs-calc-for-programmers-and-cs.html][poor man's mathematica]] does indeed seem to be true.

*** Appendix: Octave

It's of course possible to utilize [[https://orgmode.org/worg/org-contrib/babel/languages/ob-doc-octave-matlab.html][Octave]] in Org-mode as well. Here is an
example passing our synthetic data into a block
#+BEGIN_SRC octave :results output :var data=est-truth-data[1:-1, 0:1]
data
#+END_SRC
...
We can also plot with Octave and generate figures that are automatically
inserted back into the document.
#+BEGIN_SRC octave :results file :exports results
figure( 1, "visible", "off" );
sombrero;
print -dpng .images/chart.png;
ans = ".images/chart.png";
#+END_SRC

#+CAPTION: Octave cool plot
#+RESULTS:
[[file:.images/chart.png]]

That means that one can operate on the data and utilize Octave to visualize the
data for example. And with the right paramter [[https://octave.org/doc/v4.2.0/Printing-and-Saving-Plots.html][see documentation]] we can specify
